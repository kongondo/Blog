...
class BlogInstallWizard extends ProcessBlog {
		...
		*/
		...
		if ($installWizardBtn && $installWizardBtn == 'Run install wizard') { // was the right button pressed
			...
			foreach ($fieldsArray as $key => $value) {
				if ($fields->get($value)) $fieldsCheck[] = $fields->get($value)->name;
			}
			...
			foreach ($templatesArray as $tpl) {
				if ($templates->get($tpl)) $templatesCheck[] = $templates->get($tpl)->name;
			}
	...
	private function createRole() {
		...
		// @note: this is throwing error in ProcessWire 3.0.18x???
		// $r = new Role();
		// $r->name = 'blog-author';
		// $r->save();
		$this->wire('roles')->add('blog-author');
	...
	private function extraTemplateSettings() {
		...
		if ($this->blogStyle == 2 || $this->blogStyle == 4) {
			unset($childTemplates['blog-posts']);
		}
		...
		elseif ($this->blogStyle == 4) {
			$parentTemplate = $this->wire('pages')->get('/')->template->name;
		} else {
			$parentTemplate = 'blog-posts';
		} // else we default to blog-posts = blogStyle=1 || blogStyle=3
	...
	private function inContextFieldSettings() {
		...
		foreach ($userFieldsExtra as $k => $v) {
			...
			if ($k == 'title') {
				$fg->prepend($fields->get($k));
			} else {
				$fg->add($fields->get($k));
			} // images and body fields get added to the bottom
		}
